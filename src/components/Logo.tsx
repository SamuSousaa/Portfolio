...
export function Logo({ size = "sm" }: { size?: "sm" | "lg" }) {
  ...
  const lg = size === "lg";
  return (
    <TransitionLink href="/" aria-label={`${PROFILE.firstName} ${PROFILE.lastName} — ${t.a11y.home}`} data-fill className={`group flex items-center ${lg ? "gap-4" : "hit gap-3 [--hit-y:6px]"}`}>
      <LogoMark className={`w-auto shrink-0 text-fg transition-[color,translate] duration-200 group-hover:text-accent ${lg ? "h-12" : "h-8"}`} />
